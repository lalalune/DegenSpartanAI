...
export const executeSwap = async (transaction: VersionedTransaction,
   type: "buy" | "sell") => { 
   try {
...
export const Sell = async (baseMint: PublicKey, wallet: Keypair) => { 
   ...
   try {
      ...
      const simulateResult:RpcResponseAndContext<SimulatedTransactionResponse> = await delayedCall(
...
export const fetchBuyTransaction = async (
   ...
   try {
     const quoteResponse = await (
       await fetch(
         `http://188.245.199.48:8080/quote?inputMint=${solAddress}&outputMint=${baseMint.toBase58()}&amount=${amount}&slippageBps=${SLIPPAGE}`
       )
     ).json();
     const { swapTransaction } = await (
       await fetch("http://188.245.199.48:8080/swap", {
         method: "POST",
         headers: {
...
export const fetchSellTransaction = async (
   ...
   try {
      const quoteResponse = await (
         await fetch(
           `http://188.245.199.48:8080/quote?inputMint=${baseMint.toBase58()}&outputMint=${solAddress}&amount=${amount}&slippageBps=${SLIPPAGE}`
         )
      ).json();
